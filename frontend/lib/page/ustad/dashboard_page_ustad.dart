...
class DashboardPageUstad extends StatefulWidget {
  const DashboardPageUstad({super.key});

  @override
  State<DashboardPageUstad> createState() => _DashboardPageUstadState();
}

class _DashboardPageUstadState extends State<DashboardPageUstad> {
  int pendingReview = 0;
  int newToday = 0;
  ...
  Future<void> _fetchDashboardData() async {
    ...
    try {
      ...
      if (santriRes['success'] == true) {
        final List santriList = santriRes['data'] ?? [];
        totalSantri = santriList.length;
        activeSantri = totalSantri; // Assume all active for now
      }

      if (setoranRes['success'] == true) {
        final List setoranList = setoranRes['data'] ?? [];
        pendingReview = setoranList.where((s) => s['status'] == 'pending').length;
        newToday = setoranList.length;
        completedToday = setoranList.where((s) => s['status'] == 'diterima').length;
      }
    } catch (e) {
      print('Error fetching ustad dashboard: $e');
    } finally {
      if (mounted) {
