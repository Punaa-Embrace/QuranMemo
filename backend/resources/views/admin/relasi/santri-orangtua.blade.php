...
@section('content')
<div>
    <h1 class="text-2xl font-bold text-gray-800">Relasi Santri ↔ Orang Tua</h1>

    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mt-4">
        <!-- Form Assign -->
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
            <h2 class="text-lg font-semibold mb-4">Assign Santri ke Orang Tua</h2>
            <form method="POST" action="{{ route('admin.relasi.santri-orangtua.store') }}">
                @csrf
                <div class="space-y-4">
                    <div>
                    ...
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Orang Tua</label>
                        <select name="orangtua_id" class="w-full px-4 py-2 border border-gray-200 rounded-lg" required>
            ...
            <div class="space-y-2">
                ...
                <div class="flex justify-between items-center border-b border-gray-100 py-2">
                    <div>
                        ...
                        <span>{{ $r->orangtua->nama }}</span>
                    </div>
                    <form method="POST" action="{{ route('admin.relasi.santri-orangtua.destroy', $r->id) }}" onsubmit="return confirm('Hapus relasi ini?')">
                        @csrf
                        @method('DELETE')
